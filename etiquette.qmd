...
When posting job offers keep in mind that the search tool is only using hashtags,
it is therefore the most important part of your posting.

Try to follow these guidelines:

1) Use [hashtags](#use-of-hashtags), always use `#JobOffer` and a set of [standardized hashtags](#job-standardized-hashtags),
1) use `CW` on your post with a summary subject line,
1) optionally, finish by tagging any relevant group(s) such as `@jobofferecoevo@a.gup.pe` if your offer is in `#EcoEvo`.
